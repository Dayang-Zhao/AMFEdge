...
def main(dfs:list, grid:tuple, cols:list, plot_setting:dict, outpath:str):
    ...
    for i in range(nrows):
        for j in range(ncols):
            ...
            ax.text(0.05, 0.80, f"slope = {result.slope:.2f}", transform=ax.transAxes, fontsize=LABEL_SIZE+2)
...
if __name__ == "__main__":
    ...
    path2 = r"F:\Research\AMFEdge\Edge\Undist\anoVI_Amazon_undistEdge_Effect_2023.csv"
    df2 = pd.read_csv(path2)
    df = pd.merge(df1, df2, on=['Id'], suffixes=('_edge', '_undist'))
    df[['nirv_scale_edge', 'nirv_scale_undist']] = df[['nirv_scale_edge', 'nirv_scale_undist']]/1000
    dst_df = df.loc[
        (df['nirv_scale_edge']!=6.001)&(df['nirv_scale_undist']!=6.001)&
                    (~np.isnan(df['nirv_scale_edge']))&(~np.isnan(df['nirv_scale_undist']))]
    dfs = [dst_df]*2
    cols = [
        ('nirv_magnitude_undist', 'nirv_magnitude_edge', ),
        ('nirv_scale_undist', 'nirv_scale_edge')
    ]
    ...
    cmaps = [cmap2, cmap2]
    norms = [norm2, norm2]
    ylabels = [r'Sum $M_{\Delta \mathrm{NIRv}}$ (%)', r'Sum $S_{\Delta \mathrm{NIRv}}$ (km)']
    xlabels = [r'Undisturbed $M_{\Delta \mathrm{NIRv}}$ (%)', r'Undisturbed $S_{\Delta \mathrm{NIRv}}$ (km)']
    xlims = [(-8, 10), (0, 6.5)]
    ylims = [(-8, 10), (0, 6.5)]
    extend = ['both', 'max']
    titles = ['', '']
    ...
    outpath = r"E:\Thesis\AMFEdge\Figures\Comparison\corr_edge2undistEdge.pdf"
