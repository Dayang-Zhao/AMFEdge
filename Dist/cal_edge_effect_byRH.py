...
def cal_magnitude(group, y):
    # Magnitude is the difference between dist=0 and the synoptic value.
    magnitude = group.loc[group['Dist'] == 120, y].values[0]
    return magnitude
...
def cal_edge_effect(group, x:str, y:str):
    ...
    if np.isnan(scale):
        magnitude = cal_magnitude(group, y)
        return pd.Series({'para1': popt[0], 'para2': popt[1], 'para3': popt[2],
...
if __name__ == '__main__':
    path = r"F:\Research\AMFEdge\EdgeRH\dAnoVI_Amazon_UndistEdge_2023.csv"
    df = pd.read_csv(path)
    ...
    outpath = r"F:\Research\AMFEdge\EdgeRH\dAnoVI_Amazon_UndistEdge_Effect_2023.csv"
