...
def main(dfs:list, grid:tuple, cols:list, plot_setting:dict, outpath:str):
    title_nums = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i']
    nrows, ncols = grid
    fig, axes = plt.subplots(
        nrows=nrows, ncols=ncols, subplot_kw={'projection': CRS, 'frameon':True}
        )
    fig.set_size_inches(cm2inch(17), cm2inch(18))
    ...
    # Adjust.
    fig.text(0.04, 0.85, '$\Delta T_{2m}$', ha='center', va='center', rotation='vertical', fontsize=LABEL_SIZE+2)
    fig.text(0.04, 0.5, '$\Delta P$', ha='center', va='center', rotation='vertical', fontsize=LABEL_SIZE+2)
    fig.text(0.04, 0.2, '$\Delta $ET', ha='center', va='center', rotation='vertical', fontsize=LABEL_SIZE+2)
    fig.subplots_adjust(bottom=0.01, top=0.96, left=0.05, right=0.98, hspace=0.1, wspace=0.05)
...
if __name__ == "__main__":
    ...
    # Merge data.
    gdfs_merged = [gdf.merge(ave_df, on="Id", how="left") for ave_df in ave_dfs] * 3
    cols = ['temperature_2m_mean']*3 + ['total_precipitation_sum_mean']*3 + ['total_evaporation_sum_mean',]*3
    grid = (3, 3)

    # Plot setting.
    cmap1 = sns.color_palette(palette='YlOrBr', as_cmap=True)
    ...
    norm3 = mcolors.BoundaryNorm(boundaries=levels, ncolors=cmap2.N)

    cmaps = [cmap1]*3+[cmap2]*6
    norms = [norm1]*3+[norm2]*3 + [norm3]*3
    extend = ['both']*9
    titles = ['']*9

    outpath = rf"E:\Thesis\AMFEdge\Figures\CMIP6\meteo_prj_map.jpg"
