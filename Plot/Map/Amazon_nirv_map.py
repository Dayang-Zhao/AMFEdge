    ...
    for i in range(nrows):
        for j in range(ncols):
            ...
            im = dst_da.plot(ax=ax, x='lon', y='lat', cmap=cmap, add_colorbar=True, vmin=vrange[0], 
                    ...
                    cbar_kwargs={'label':'$\Delta$NIRv (%)','pad':0.02, 'shrink':0.9, 'aspect':20, 'extend':'neither',
...
if __name__ == '__main__':
    # Read data
    paths = ["F:\Research\AMFEdge\Meteo\Amazon_2023_S2AnoVI.nc"]
    ds = xr.open_dataset(paths[0]).drop('spatial_ref')
    dst_var = 'NIRv'
    ds['NIRv'] = ds['NIRv']/100
    ds = ds.where(ds[dst_var]!= 0)
    dst_vars = ['NIRv']
    ds_array = [ds]
    ...
    outpath = "E:\Thesis\AMFEdge\Figures\Description\Amazon_2023_S2AnoVI.png"
