    ...
    for i in range(nrows):
        for j in range(ncols):
            ...
            df.plot(ax=ax, column=col, cmap=cmap, edgecolor='#919191', 
                    ...
                    legend_kwds={"shrink": 0.9, 'orientation':'horizontal', 'location':'bottom', 'pad':0.02, 'extend':'both'},
...
if __name__ == "__main__":
    ...
    path1 = r"F:\Research\AMFEdge\Edge\anoVI_Amazon_UndistForest_2023_Grid.csv"
    df1 = pd.read_csv(path1)
    df1['NIRv'] = df1['NIRv'].apply(lambda x: ast.literal_eval(x.replace('null', '0'))[2])
    path2 = r"F:\Research\AMFEdge\Meteo\Amazon_2023_droughtPeriod.csv"
    df2 = pd.read_csv(path2)

    # Merge data.
    gdf_merged = gdf.merge(df1, on="Id", how="left")
    gdf_merged = gdf_merged.merge(df2[['Id', 'length_mean']], on="Id", how="left")
    dfs = [gdf_merged]*2
    cols = ['NIRv', 'length_mean']
    grid = (1, 2)
