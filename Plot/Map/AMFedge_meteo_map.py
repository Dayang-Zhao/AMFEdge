    ...
    for i in range(nrows):
        for j in range(ncols):
            ...
            df.plot(ax=ax, column=col, cmap=cmap, edgecolor='#919191', 
                    ...
                    legend_kwds={"shrink": 0.8},missing_kwds={'color': 'lightgrey'})
...
if __name__ == "__main__":
    ...
    csv_path = r"F:\Research\AMFEdge\Edge\anoVI_Amazon_UndistEdge_2023.csv"
    df = pd.read_csv(csv_path)
    dst_df = df.loc[df['Dist']==-1]

    # Merge data.
    gdf_merged = gdf.merge(dst_df, on="Id", how="left")
    dfs = [gdf_merged]*2
    cols = ['anoMCWD_mean', 'NIRv_mean']
    grid = (1, 2)
    ...
    cmaps = [cmap1, cmap2]
    norms = [norm1, norm2]
    titles = ['$\Delta$MCWD (%)', '$\Delta$NIRv (%)']

    outpath = r"E:\Thesis\AMFEdge\Figures\Description\meteo_map.pdf"
