...
def main(model, raw_df:pd.DataFrame, xcols:list, features:list, grid:tuple, plot_setting:dict, outpath:str):
    ...
    for i in range(nrows):
        for j in range(ncols):
            ...
            pos_df = raw_df[raw_df[ycol] >= 0]
            neg_df = raw_df[raw_df[ycol] < 0]

            # Plot.
            if j == 1:
                # neg_df = raw_df[raw_df[ycol] < 0]
                disp = PartialDependenceDisplay.from_estimator(
                    model, df[xcols], features=[xcol], kind="individual", grid_resolution=50,
                ...
                # Linear regression line and r value.
                result = linregress(pos_df[xcol], pos_df[ycol])
                x = np.linspace(xlim[0], xlim[1], 100)
                y = result.intercept + result.slope * x
                ax.plot(x, y, color='black', linewidth=1.5, linestyle='--')
                ax.text(0.1, 0.8, '*$r$= '+str(result.rvalue.round(2)), fontsize=12, transform=ax.transAxes)

                result = linregress(neg_df[xcol], neg_df[ycol])
                x = np.linspace(xlim[0], xlim[1], 100)
                y = result.intercept + result.slope * x
                ax.plot(x, y, color='black', linewidth=1.5, linestyle='--')
                ax.text(0.6, 0.08, '**$r$= '+str(result.rvalue.round(2)), fontsize=12, transform=ax.transAxes)
...
if __name__ == "__main__":
    ...
    # df = df[df['Id'].isin(gv.SWGRID_IDS)]
    X = df[xcols]
    y = df[ycol]
    ...
    # Plot
    grid = (1,2)
    features = [('rh98_magnitude', 'nirv_magnitude','nirv_magnitude', 'nirv_scale'),]*2
    ...
    plot_setting = {
        ...
        'xlabels': ['$M_{\mathrm{RH98}}$ (m)']*2,
        'ylabels': [r'$M_{\Delta \mathrm{NIRv}}$ (%)']*2,
        'xlims': [[0, 14], [2.5, 12]],
        'ylims': [[-8.5, 13], [-7, 12]],
        'cmaps': cmaps,'norms': norms, 'levels': [levels]*2
    }
    outpath = r'E:\Thesis\AMFEdge\Figures\Cause\Mnirv_RH98.pdf'
    main(model, df, xcols, features, grid, plot_setting, outpath=outpath)
