...
if __name__ == "__main__":
    ...
    df[['nirv_scale', 'evi_scale']] = df[['nirv_scale', 'evi_scale']]/1000
    dst_df = df.loc[
        (df['nirv_scale']!=6.001)&(df['evi_scale']!=6.001)&
                    (~np.isnan(df['nirv_scale']))&(~np.isnan(df['evi_scale']))]
    dfs = [dst_df]*2
    cols = [
        ('nirv_magnitude', 'evi_magnitude'),
        ('nirv_scale', 'evi_scale')
    ]
    ...
    cmaps = [cmap2, cmap2]
    norms = [norm2, norm2]
    xlabels = [r'$M_{\Delta \mathrm{EVI}}$ (%)', r'$S_{\Delta \mathrm{EVI}}$ (km)']
    ylabels = [r'$M_{\Delta \mathrm{NIRv}}$ (%)', r'$S_{\Delta \mathrm{NIRv}}$ (km)']
    xlims = [(-8, 10), (0, 6.5)]
    ylims = [(-8, 10), (0, 6.5)]
    extend = ['both', 'max']
    titles = ['', '']
    ...
    outpath = r"E:\Thesis\AMFEdge\Figures\Comparison\corr_nirv2evi.pdf"
