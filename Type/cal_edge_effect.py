...
def cal_edge_dmagnitude(edge_df, spec_edge_df, dst_vars):
    ...
    for dst_var in dst_vars:
        ...
        outdfs.append(dst_merge_df[['Id', 'type', dst_var+'_mean', 'intact_' + dst_var.lower(), 'd'+dst_var+'_mean']])

    outdf = reduce(lambda left, right: pd.merge(left, right, on=['Id','type'], how='outer'), outdfs)
    return outdf

if __name__ == '__main__':
    path = r"F:\Research\AMFEdge\EdgeType\anoVI_Amazon_specUndistEdge_2023.csv"
    df = pd.read_csv(path)
    ...
    # Save.
    outpath = r"F:\Research\AMFEdge\EdgeType\anoVI_Amazon_specUndistEdge_effect_2023.csv"
