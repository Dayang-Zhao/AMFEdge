    ...
    for i in range(nrows):
        for j in range(ncols):
            ...
            df.plot(ax=ax, column=col, cmap=cmap, edgecolor='#919191', 
                    linewidth=0.3, legend=True, norm=norm,
...
if __name__ == "__main__":
    ...
    dfs = [gdf_merged]*2
    cols = ['ndwi_magnitude', 'ndwi_scale']
    grid = (1, 2)
    ...
    norms = [norm2, norm1]
    extend = ['both', 'max']
    titles = ['$\Delta$NDWI Magnitude (%)', '$\Delta$NDWI Scale (km)']

    outpath = r"E:\Thesis\AMFEdge\Figures\Edge\NDWI_edge_map.pdf"
