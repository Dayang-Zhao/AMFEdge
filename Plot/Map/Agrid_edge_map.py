...
def main(dfs:list, grid:tuple, cols:list, plot_setting:dict, outpath:str):
    ...
    fig.set_size_inches(cm2inch(7.5), cm2inch(8))
    ...
    for i in range(nrows):
        for j in range(ncols):
            ...
            df.plot(ax=ax, column=col, cmap=cmap, edgecolor='#919191', 
                    ...
                    legend_kwds={"shrink": 0.8, 'extend':'max', 'orientation':'horizontal', 'location':'bottom',
                                 'label':title, 'pad': 0.02},missing_kwds={'color': 'white'})

            # Plugins.
            ax.set_title(title_num, loc='left', fontsize=LABEL_SIZE+1)
    ...
    # Adjust.
    # Adjust.
    fig.subplots_adjust(bottom=0.05, top=0.92, left=0.0, right=0.99, hspace=0, wspace=0.05)
...
if __name__ == "__main__":
    ...
    csv_path = r"F:\Research\AMFEdge\EdgeNum\Area_Amazon_Edge_2023.csv"
    ...
    cmaps = [cmap1, cmap2]
    norms = [norm1, norm2]
    titles = ['Edge forest proportion (%)', '$d_{95}$ (km)']

    outpath = r"E:\Thesis\AMFEdge\Figures\Description\Amazon_edgeFrac_map.pdf"
